...
- Model K-Nearest Neighbors dibangun menggunakan KNeighborsClassifier dari library scikit-learn  
- parameter n_neighbors=5 sebagai default, yang berarti model akan mempertimbangkan 5 tetangga terdekat untuk menentukan kelas
...
- Model Random Forest dibangun menggunakan RandomForestClassifier dari scikit-learn.
- Dengan parameter n_estimators=100 (sebagai nilai default), berarti model menggunakan 100 pohon keputusan, lebih banyak pohon bisa meningkatkan performa tetapi juga meningkatkan waktu komputasi.
- Parameter random_state=42 digunakan agar hasil model bersifat reproducible (konsisten setiap kali dijalankan).
- Model ini kemudian dilatih menggunakan data pelatihan X_train dan y_train.
...
- Model XGBoost dibangun menggunakan XGBClassifier
- Parameter yang digunakan adalah use_label_encoder=False, sebagai nilai false yang digunakan untuk menonaktifkan encoder label bawaan (karena depreceted)
- Parameter eval_metric='logloss',sebagai metrik evaluasi selama pelatihan.
...
🎯**Goal 1** :  Untuk mengetahui fitur apa saja yang berpengaruh terhadap loan_status sehingga dapat mengambil keputusan selanjutnya, untuk melakukan prediksi status pinjaman berdasarkan riwayat calon peminjam.
...
🎯**Goal 2** :
Untuk melakukan analisis dan membandingkan performa beberapa model seperti KNN, Random Forest, dan XGBoost untuk memilih model dengan kinerja terbaik dalam memprediksi status pinjaman berdasarkan riwayat calon peminjam.
